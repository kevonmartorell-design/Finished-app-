...
function SidebarNav() {
  ...
  const navItems = [
    ...
    { label: 'Schedule', path: '/(business)/schedule/', icon: '📅' },
    { label: 'Team', path: '/(business)/team/', icon: '👥' },
    { label: 'Messages', path: '/(shared)/messages/', icon: '💬' },
    { label: 'Timesheets', path: '/(business)/timesheets/', icon: '⏱' },
    { label: 'Payroll', path: '/(business)/payroll', icon: '💰' },
    { label: 'Tasks', path: '/(business)/tasks/', icon: '✓' },
    { label: 'Reports', path: '/(business)/reports/', icon: '📊' },
    { label: 'Departments', path: '/(business)/departments', icon: '🏢' },
    { label: 'Audit Log', path: '/(business)/audit-log', icon: '📋' },
...
function MoreMenu({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  const menuItems = [
    { label: 'Timesheets', path: '/(business)/timesheets/' },
      ...
      >
        ...
        <Tabs.Screen
          ...
          listeners={{
            tabPress: (e) => {
              e.preventDefault();
              router.push('/(shared)/messages/' as never);
            },
          }}
        />
        <Tabs.Screen
